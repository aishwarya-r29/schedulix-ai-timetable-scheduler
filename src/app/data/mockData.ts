...
export interface Faculty {
  ...
  subjects: string[];
  isHOD?: boolean;
  maxDailySlots?: number;   // generator constraint (default 5, HOD default 3)
  offSlots?: string[];      // preferred off-slots e.g. ["Monday_5", "Friday_8"]
}
...
const parseStudentsFromCsv = (csv: string): Student[] => {
  ...
  return rows.map(row => {
    ...
    return {
      ...
      email,
      password,
      department: department as 'CSE' | 'IT',
    };
  });
};

export const STUDENTS: Student[] = parseStudentsFromCsv(RAW_STUDENT_CSV);
...
export const CLASSROOMS: Classroom[] = [
  { id: 'cr1',  classroomNumber: 'CS-101',  capacity: 60, status: 'available', roomType: 'theory', departmentPriority: 'CSE' },
  { id: 'cr2',  classroomNumber: 'CS-102',  capacity: 60, status: 'available', roomType: 'theory', departmentPriority: 'CSE' },
  { id: 'cr3',  classroomNumber: 'CS-103',  capacity: 60, status: 'available', roomType: 'theory', departmentPriority: 'CSE' },
  { id: 'cr4',  classroomNumber: 'CS-104',  capacity: 60, status: 'available', roomType: 'theory', departmentPriority: 'CSE' },
  { id: 'cr5',  classroomNumber: 'IT-201',  capacity: 60, status: 'available', roomType: 'theory', departmentPriority: 'IT' },
  { id: 'cr6',  classroomNumber: 'IT-202',  capacity: 60, status: 'available', roomType: 'theory', departmentPriority: 'IT' },
  { id: 'cr7',  classroomNumber: 'IT-203',  capacity: 60, status: 'available', roomType: 'theory', departmentPriority: 'IT' },
  { id: 'cr8',  classroomNumber: 'IT-204',  capacity: 60, status: 'available', roomType: 'theory', departmentPriority: 'IT' },
  { id: 'cr9',  classroomNumber: 'LAB-301', capacity: 50, status: 'available', roomType: 'lab',    departmentPriority: 'Both' },
  { id: 'cr10', classroomNumber: 'LAB-302', capacity: 50, status: 'available', roomType: 'lab',    departmentPriority: 'Both' },
  { id: 'cr11', classroomNumber: 'LAB-303', capacity: 50, status: 'available', roomType: 'lab',    departmentPriority: 'Both' },
  { id: 'cr12', classroomNumber: 'LAB-304', capacity: 50, status: 'available', roomType: 'lab',    departmentPriority: 'Both' },
];

// Groups (derived from student assignments; CSE 200 students → G1/G2 each 100)
...
export const addTimetable = (timetable: Timetable) => {
  // Replace if one already exists for the same section
  const idx = TIMETABLES.findIndex(tt => tt.department === timetable.department && tt.section === timetable.section);
  if (idx >= 0) TIMETABLES[idx] = timetable;
  else TIMETABLES.push(timetable);
};
...
// Helper: derive roomType from classroomNumber
export const deriveRoomType = (classroomNumber: string): 'theory' | 'lab' =>
  classroomNumber.toUpperCase().startsWith('LAB') ? 'lab' : 'theory';
